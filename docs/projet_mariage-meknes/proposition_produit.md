...
22. **Auto-ajout d'un invité au RSVP si son numéro n'est pas dans la liste importée, mais a un format plausible FR/MA/US** (2026-07-28) — après la porte d'entrée (code commun), si `/rsvp/lookup` ne trouve aucun `household_member` pour le numéro saisi : rejeté avec l'erreur habituelle si le format ne ressemble à rien (FR/MA/US, `app/phone.py::is_plausible_phone` — accepte aussi de facto les numéros canadiens, qui partagent le format NANP des numéros US) ; sinon, un écran demande nom/prénom et crée un nouveau foyer d'un seul membre. Nouveau champ `household_member.statut` (`liste_origine` | `ajoute_rsvp`) pour distinguer les deux origines en base — utile pour un futur écran admin ou une relecture. Pas de vérification d'identité au-delà du format du numéro.

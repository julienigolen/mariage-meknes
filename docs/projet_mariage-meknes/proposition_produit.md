...
22. **Auto-ajout d'un invité au RSVP si son numéro n'est pas dans la liste importée** (2026-07-28, **filtre de format retiré le 2026-08-01**) — après la porte d'entrée (code commun), si `/rsvp/lookup` ne trouve aucun `household_member` pour le numéro saisi, un écran demande nom/prénom et crée un nouveau foyer d'un seul membre. Nouveau champ `household_member.statut` (`liste_origine` | `ajoute_rsvp`) pour distinguer les deux origines en base — utile pour un futur écran admin ou une relecture. Pas de vérification d'identité au-delà de la présence d'un numéro et d'un nom. **2026-08-01 (bug Patron)** : la V1 rejetait avec l'erreur habituelle tout numéro ne ressemblant pas à un format FR/MA/US (`app/phone.py::is_plausible_phone`) — un invité entré via le code commun avec un numéro dans un format non couvert se heurtait donc à tort à l'erreur au lieu d'accéder à la saisie du nom. La liste importée n'étant jamais garantie exhaustive, le filtre de format est retiré : tout numéro non trouvé mène désormais à l'écran nom/prénom, sans condition de format. `is_plausible_phone` supprimée (plus d'appelant).

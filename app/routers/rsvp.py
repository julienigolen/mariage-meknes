...
@router.get("/rsvp")
def rsvp_page(request: Request):
    if (r := gate_redirect(request)) is not None:
        return r
    return templates.TemplateResponse(
        request, "rsvp.html", {"step": "phone", "error": False, **lang_context(request)}
    )
...
def rsvp_lookup(request: Request, phone: str = Form(""), db: Session = Depends(get_db)):
    ...
    member = db.execute(
        select(HouseholdMember).where(HouseholdMember.phone == normalized)
    ).scalar_one_or_none()
